...
object TimersSchedulers extends App {
  ...
  system.scheduler.scheduleOnce(2 seconds) {
    system.log.info("Sending ping to the self closing actor")
    selfClosingActor ! "ping"
  }
  ...
  case object Stop
  class TimerBasedCActor extends Actor with ActorLogging with Timers {
  ...
  val timerHeartbeatActor = system.actorOf(Props[TimerBasedCActor], "timerActor")
  system.scheduler.scheduleOnce(5 seconds) {
